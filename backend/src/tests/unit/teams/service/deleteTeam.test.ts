...
describe('deleteTeam()', () => {
  test('존재하지 않는 팀 ID일 경우 404 에러를 반환해야 한다', async () => {
    ...
    await expect(deleteTeam(testConnection, { teamId: 9999, ownerId: 1 })).rejects.toThrowError(
  ...
  test('팀장이 아닌 사용자가 삭제를 시도할 경우 403 에러를 반환해야 한다', async () => {
    ...
    await expect(
      deleteTeam(testConnection, { teamId: 1, ownerId: 99 }) // 요청자는 팀장이 아님
  ...
  test('정상적으로 팀을 삭제하면 팀 이름을 반환해야 한다', async () => {
    ...
    const result = await deleteTeam(testConnection, { teamId: 1, ownerId: 10 });
