...
describe('getTeamMembers()', () => {
  test('존재하지 않는 팀일 경우 404 에러를 반환해야 한다', async () => {
    ...
    await expect(
      getTeamMembers(testConnection, { teamId: 1, requesterId: 10 })
  ...
  test('요청자가 팀장이 아닌 경우 403 에러를 반환해야 한다', async () => {
    ...
    await expect(
      getTeamMembers(testConnection, { teamId: 1, requesterId: 99 })
  ...
  test('정상적으로 팀 멤버 목록을 반환해야 한다', async () => {
    ...
    const result = await getTeamMembers(testConnection, {
      teamId: 1,
      requesterId: 10,
    });
